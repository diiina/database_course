...
pu AS
(
  SELECT CAST(payment_dttm AS DATE) AS dt, COUNT(DISTINCT payments.user_id) AS p
  FROM payments 
  GROUP BY dt
),
p_sum AS
(
  SELECT CAST(payment_dttm AS DATE) AS dt, SUM(DISTINCT payments.payment_sum) AS sum
  FROM payments 
  GROUP BY dt
)
SELECT cte.dt, COALESCE(p_sum.sum, 0)/COALESCE(pu.p, 1) AS ARPPU
FROM cte
LEFT JOIN pu ON cte.dt=pu.dt
LEFT JOIN p_sum ON cte.dt=p_sum.dt;
